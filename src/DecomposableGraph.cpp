...
DecomposableGraph::DecomposableGraph(string filename, bool debug) : AdjacencyList(filename, debug), densest_prefix(),
                                                                    decomposed(false), minHeap(this->num_nodes) {
...
int DecomposableGraph::decomposeGraph(bool debug) {
    ...
    while (!minHeap.empty()) {
        ...
        for (unsigned int i = 0; i < v->degree; i++) {
            ...
            if (neigh < 1) {
                cout << "[ERROR] - DecomposableGraph::decomposeGraph(): neighbour #" << i << " of " << v->ID
                     << " not found... Aborting...";
                return -1;
            }
...
int DecomposableGraph::writeCorenessDegreeFile(const string filename, const bool debug) const {
    ...
    if (ofile.is_open()) {
        ...
        while (p <= num_nodes) {
            ...
            if (debug)
                cout << "[DEBUG] - DecomposableGraph::writeCorenessDegreeFile(): parsing " << coreness << "-core..."
                     << endl;
            ...
            if (debug)
                cout << "[DEBUG] - DecomposableGraph::writeCorenessDegreeFile(): writing [d c o], occ_size: "
                     << occ_size << endl;
...
void DecomposableGraph::findDensestPrefixWithMKscore(bool debug) {
    mkscore(100, debug);
    ...
    while (!scoreMinHeap.empty()) {
        ...
        if (debug)
            cout << "[DEBUG] - DecomposableGraph::findDensestPrefixWithMKscore(): ID:" << ordered_n[n]->ID
...
int DecomposableGraph::writeAllInFile(const string filename, const bool debug) {
    ...
    float perc;
    unsigned resolution;
    const int start = (int)time(nullptr);
...
void DecomposableGraph::update(int i) {
    minHeap.update(i);
}

void DecomposableGraph::heapTest() {
...
void DecomposableGraph::print_prefix(unsigned int p) {
    cout << "prefix:             ";
    if (!p)
        p=num_nodes;
...
void DecomposableGraph::flush_minHeap() {
    minHeap.flush_heap();
}

void DecomposableGraph::print_minHeap_container() {
    minHeap.print_container();
}
